...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setMessage(error ? error.message : tx.success)
    setLoading(false)
  }
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl p-6" style={{ background: 'var(--card-bg)', border: '1px solid var(--border)' }}>
          <button onClick={handleGoogleLogin} className="w-full py-3 rounded-xl border text-sm font-medium mb-4 flex items-center justify-center gap-2 transition-colors hover:bg-gray-50 cursor-pointer" style={{ borderColor: 'var(--border)' }}>
          ...
          <div className="relative mb-4">
            ...
            <div className="relative flex justify-center text-xs"><span className="px-2" style={{ background: 'var(--card-bg)', color: 'var(--subtext)' }}>{tx.or}</span></div>
          </div>
          <form onSubmit={handleSignup} className="flex flex-col gap-3">
            ...
            <input type="password" placeholder={tx.password} value={password} onChange={e => setPassword(e.target.value)} minLength={8} required className="w-full px-4 py-2.5 rounded-xl border text-sm outline-none" style={{ borderColor: 'var(--border)' }} />
            {message && <p className={`text-xs ${message.includes('確認') || message.includes('Check') ? 'text-green-600' : 'text-red-500'}`}>{message}</p>}
          ...
          <p className="text-center text-xs mt-4" style={{ color: 'var(--subtext)' }}>
            {tx.hasAccount} <Link href="/login" style={{ color: 'var(--accent)' }}>{tx.login}</Link>
          </p>
        </div>
      </div>
    </div>
